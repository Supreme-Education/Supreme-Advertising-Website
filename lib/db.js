...
function ensureCustomerColumn(name, ddl) {
  const columns = db.prepare("PRAGMA table_info(customers)").all();
  if (!columns.some((col) => col.name === name)) db.exec(ddl);
}
ensureCustomerColumn("company", "ALTER TABLE customers ADD COLUMN company TEXT");
...
ensureDocumentColumn("client_company", "ALTER TABLE documents ADD COLUMN client_company TEXT");
...
function listCustomers({ search } = {}) {
  ...
  const params = [];
  if (search) {
    sql += " AND (name LIKE ? OR email LIKE ? OR company LIKE ? OR address LIKE ?)";
    const q = `%${search}%`;
    params.push(q, q, q, q);
  }
...
const { normalizeCustomerPayload } = require("./document-normalize");
