...
const { companyFromEmail } = require("./company-from-email");

/** @param {object} store - document/customer store (no db import in this file) */
function buildDataApp(store, { serverless = false } = {}) {
  const loadDb = () => store;
  const app = express();
  ...
  app.get("/api/company-from-email", requireAuth, (req, res) => {
    res.json({ company: companyFromEmail(req.query.email || "") });
  });
