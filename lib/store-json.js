...
/** Bump when seed data changes — forces reload from bundled seed on deploy. */
...
function normalizeSeedState(state) {
  if (!state || !Array.isArray(state.customers)) return defaultState();
  state.seedVersion = SEED_VERSION;
  state.customers = state.customers.map((c) => ({
    ...c,
    company: c.company == null ? "" : String(c.company),
  }));
  if (!state.nextId) state.nextId = { document: 1, customer: 1 };
  return state;
}

function loadSeedFile() {
  try {
    return normalizeSeedState(require("./admin-seed.json"));
  } catch (err) {
    console.error("Failed to load bundled admin-seed.json:", err);
  }
  const fallbacks = [
    path.join(__dirname, "admin-seed.json"),
    path.join(__dirname, "..", "data", "admin-seed.json"),
    path.join(process.cwd(), "data", "admin-seed.json"),
  ];
  for (const filePath of fallbacks) {
    try {
      if (fs.existsSync(filePath)) {
        return normalizeSeedState(JSON.parse(fs.readFileSync(filePath, "utf8")));
      }
    } catch {
      /* try next */
    }
...
function hasCustomers(state) {
  return Boolean(state && Array.isArray(state.customers) && state.customers.length > 0);
}

function shouldUseStoredState(state) {
  return (
    hasCustomers(state) &&
    Number(state.seedVersion || 0) >= SEED_VERSION
  );
}
...
async function initStore() {
  ...
  initPromise = (async () => {
    ...
    console.log(
      `Admin store loaded seed v${SEED_VERSION}: ${stateCache.customers.length} customers`
    );
    return stateCache;
  })();
